...
public class UserDAO extends DAO<User> {
	...
	public User login(String username, String password) {
		...
		for (Iterator<User> it = this.getAll(); it.hasNext();) {
			User userIt = it.next();
			if (userIt.username().equals(username) && userIt.password().equals(password)) {
				return userIt;
			}
		}
		return null;
	...
	public User register(String username, String password) {
		// TODO: Complete this method, interfacing with the DAO pattern, to the specification in the javadoc
		if (!username.matches("^[A-Za-z0-9]+$") || username.length() > 20 || username.length() < 4) {
			return null;
		}
		...
		for (Iterator<User> it = this.getAll(); it.hasNext();) {
			User userIt = it.next();
			if (userIt.username().equals(username)) {
				return null;
			}
		}
